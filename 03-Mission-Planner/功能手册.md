...
5. 添加盘旋点：  
   ...
   - 高度设为 80  
6. 固定翼降落设置（推荐）：  
   - **不要使用 `RETURN_TO_LAUNCH`（RTL）作为最后一步**  
   - 在 HOME 附近加一个航点作为进近点  
   - 最后一条命令设为 `LAND`（降落点可设在 HOME 位置）  
   - 也可以选中 HOME/起飞点，右键选择“降落”自动插入 LAND  
   - 固定翼需按航线进近降落；多旋翼可垂直降落  
...
在 SITL 中规划并执行完整航线，满足以下要求：
1) 起飞（TAKEOFF）  
2) 至少 3 个航点  
3) 一次盘旋（LOITER）  
4) 返航（RTL / RETURN_TO_LAUNCH）

### 6.2 前置条件

- SITL 已启动（`sim_vehicle.py -v ArduPlane --console --map`）
- Mission Planner 已连接（右上角显示 `UDP 115200` 与 `UDP14550-1-FIXED WING`）
- Flight Plan 已能正常添加航点
...
1. 点击 `写入航点`（Write WPs）写入飞控  
2. 点击 `读取航点`（Read WPs）确认写入成功  
   - 弹窗提示“清空现有航点”属正常提示  
3. 切换飞行模式为 `AUTO`  
4. 解锁（ARM）并开始执行任务
...
### 6.7 实战记录截图（2026-01-23）

截图目录：`docs/03-Mission-Planner/20260123飞行记录/`

- 航线规划界面：  
  ![航线规划界面](./20260123飞行记录/航线规划界面20260123.png)
- 盘旋执行（2 圈）：  
  ![绕航点飞行2圈](./20260123飞行记录/绕航点飞行2圈.png)
- 降落阶段：  
  ![降落](./20260123飞行记录/降落.png)
- 降落后再起飞（记录）：  
  ![降落后再起飞](./20260123飞行记录/降落后再起飞.png)

### 6.8 航线文件保存

本次航线文件保存位置：  
`docs/06-实践记录/SITL/20260120 1st fly.waypoints`
