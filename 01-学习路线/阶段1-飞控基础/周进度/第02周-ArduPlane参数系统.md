...
### 完成情况
- [x] 《核心参数速查表》 ✅ 完整 4 大类参数 + 调参经验总结
- [x] 调参日志（≥3 次实验） ✅ PID/TECS/导航/安全多组实验
- [x] 理解参数影响 ✅ 建立参数与物理力矩的映射关系

### 额外完成
- [x] 《小型无人机理论与控制》第 4-5 章
- [x] 产出 4 份实战指南文档（PID/TECS/导航/安全）
- [x] 建立理论（稳定性导数）与实践（PID调参）的联系

### 主要收获
1. **理论与实践结合**：P 对应恢复力矩（稳定性导数效能），D 对应阻尼（阻尼导数效能）
2. **TECS 原理**：高度=势能，空速=动能，俯仰管空速趋势，油门管总能量
3. **L1 导航**：周期越小转弯越紧但易过冲，阻尼越大越平滑
4. **调参方法论**：单变量实验、A/B 对照、曲线记录

### 遇到的问题
1. I 项干扰掩盖低 P 响应 → 通过隔离参数（I/D 设为 0）解决
2. 极高增益导致空速计失效报错 → 确认飞控一致性检查保护机制
3. 部分参数名在新固件中有变化（如 `LIM_ROLL_CD` → `ROLL_LIMIT_DEG`）

### 对下周的建议
- 书籍学习进度超前，可以继续学习第 6 章（传感器）
- 坐标系与姿态已在书中学过，可侧重 Python 可视化实现
- 为第 4 个月的卡尔曼滤波/EKF 做准备
